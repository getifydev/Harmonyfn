...
// Type: Harmony.Frontend.Services.ConfigService
...
using Haromny.Frontend.Enums;
using Haromny.Frontend.Models.Config;
using Haromny.Frontend.Utilities;
using System;
...
#nullable enable
namespace Haromny.Frontend.Services
{
  public class ConfigService
  {
    ...
    public async Task LoadConfiguration()
    {
      ...
      string path = Path.Combine(Strings.CONFIG_PATH, "Harmony.conf");
      if (!File.Exists(path))
      {
    ...
    public async Task SaveConfiguration()
    {
      ...
      await File.WriteAllTextAsync(Path.Combine(Strings.CONFIG_PATH, "Harmony.conf"), contents);
    }
    ...
    public async Task<int> ValidateConfiguration()
    {
      ...
      {
        ++num;
        Logger.Log("ValidateConfiguration: DisplayName is null or empty! Reverting to \"Harmony\"", LogCategory.ConfigurationService, LogType.Warning);
        this.Configuration.DisplayName = "Harmony";
      }
