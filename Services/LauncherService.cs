...
// Type: Harmony.Frontend.Services.LauncherService
// Assembly: Hamrony, Version=2.1.0.4, Culture=neutral, PublicKeyToken=null
...
using Harmony.Backend.Controllers;
...
namespace Harmony.Frontend.Services
{
  public class LauncherService
  {
    ...
    public async Task StartGame(Installation installation)
    {
      ...
      McpController.OnHarmonyRequested += new Action(launcherService.OnHarmonyRequested);
      ...
      tempPath = Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Harmony");
      ...
      else
      {
        ...
        {
          "-AUTH_LOGIN=" + launcherService._configService.Configuration.DisplayName + "@Harmonyfn.xyz",
        ...
        string str2 = Assembly.GetExecutingAssembly().Location.Replace("Harmony.dll", "");
    ...
    public async Task StartGameWithLuna(Installation installation)
    {
      ...
      launcherService._richPresenceService.SetPlayingFortnite(installation);
      if (LauncherService.ShowToastNotification)
        new ToastContentBuilder().AddText("Sending you into Harmony Multiplayer!").AddText("Please note that you will need to log in with a Luna account. For instructions on how to make one, join our Discord Server!").Show();
      int num1 = await launcherService._configService.ValidateConfiguration();
      Logger.Log("DisplayName: " + launcherService._configService.Configuration.DisplayName, LogCategory.LauncherService);
      gamePath = installation.Path + "\\FortniteGame\\Binaries\\Win64\\FortniteClient-Win64-Shipping.exe";
      tempPath = Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Harmony");rift
      if (!System.IO.File.Exists(gamePath))
      {
        Logger.Log("The game path didn't have a Fortnite version installed!", LogCategory.LauncherService, LogType.Error);
        int num2 = (int) MessageBox.Show("The path you provided doesn't have Fortnite installed! Make sure the path points to the folder that contains the FortniteGame and Engine folders.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
    ...
    private void OnHarmonyRequested()
    {
      Logger.Log("Gluing Moon", LogCategory.Injector);
      LauncherService.Glue(this._fortniteProcess.Handle, Assembly.GetExecutingAssembly().Location.Replace("Harmony.dll", "") + "Moon.dll");
    }
    ...
    public async Task<bool> RegisterGame(string name, string path)
    {
      ...
      {
        string str = await this._jsRuntime.InvokeAsync<string>("Harmony.modalManager.hideModal", (object) "new-game");
      }
